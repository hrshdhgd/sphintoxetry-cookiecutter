...
This kickstarts an interactive session where you declare the following:
 ...
 - <sup>Caution</sup>`github_token_variable_name_for_doc_deployment`: The github token **variable name** for document deployment using `Sphinx`. [defaults to: `GH_TOKEN`]
 - <sup>Caution</sup>`github_token_variable_name_for_pypi_deployment`: The github token **variable name** which aligns with your autogenerated PyPI token for making releases. [defaults to: `PYPI_TOKEN`]

<sup>Caution</sup> **NOTE: Do NOT enter actual token here, this is just the variable name that holds the token value in the project repository's Secrets.**
